# Paradise Law AZ — Greenfield Product Requirements (2025)

This PRD is self‑contained for a new team to implement the website and back‑office workflows from scratch. It includes goals, scope, flows, tech stack, data model, endpoints, security, accessibility, analytics, and a phased delivery plan.

## 1) Executive Summary
- Primary Goal: Sell a 90‑minute virtual consultation that includes a written follow‑up action plan.
- Secondary Goal: Serve as the firm’s primary family‑law site to acquire limited‑scope and full‑scope representation.
- Constraints:
  - Manual conflict check must occur before any payment request.
  - Payments via LawPay (not Stripe).
  - Scheduling via Calendly (initially embed; API added later); time is tentatively held until payment.
  - Comprehensive questionnaire + uploads required ≥24h prior to consult.
  - MyCase is the system of record after conversion (no API initially; fallback via intake email/Zapier; API adapter later).
...
- Representation conversion: ≥25% of consults → limited/full‑scope within 30 days.
...
## 5) Key Flows
### 5.1 Consult Flow (Manual Conflict Gate)
1) User selects slot in Calendly (tentative).  
2) Site collects conflict info: client legal name, opposing party names, county, brief matter type.  
3) Staff reviews conflict in Admin console (reference MyCase/internal sources).  
4) Decision:
   - Clear → Email LawPay payment link + questionnaire link; status: “Clear – Pending Payment”.
   - Flagged → Decline email; auto‑cancel Calendly event via API; log rationale (internal only).
...
6) Questionnaire: Autosave/resume link; dynamic branching; uploads; required ≥24h before consult (immediate for short‑notice).
...
- Pick service (inclusions/exclusions, fixed pricing) → Manual conflict review → If Clear: LawPay link + tailored intake → Confirmation + delivery milestones.

### 5.3 Full‑Scope Representation (FSR)
- Lead form (or post‑consult upgrade) → Manual conflict review → If Clear: retainer to trust via LawPay + e‑sign engagement → Onboarding questionnaire & docs.

## 6) Non‑Functional Requirements
- Performance: LCP <2.5s; CLS <0.1; INP good; third‑party scripts only where needed; reserve widget heights; use View Transitions.
- Accessibility: WCAG 2.1 AA; keyboard operability; aria‑labels/aria‑describedby; aria‑hidden for decorative icons; respect prefers‑reduced‑motion.
...
- Calendly embed + conflict capture form; BookingIntent + ConflictRecord; Admin console (basic queue/actions).  
...
- Env Vars (examples):
  ...
  - `LAWPAY_MERCHANT_ID`, `LAWPAY_HMAC_SECRET` (or equivalent)
  - `MYCASE_INTAKE_EMAIL` (Phase 1), `MYCASE_API_KEY` (Phase 3)
  - `NEXTAUTH_SECRET` (if using NextAuth), `APP_BASE_URL`
